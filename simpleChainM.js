...
class Blockchain{
  ...
  addBlock(newBlock){
    ...
    .then(function(height){
      ...
      // Block height
      newBlock.height = Number(height) +1;
      console.log('newBlock.height= '+newBlock.height);

      // previous block hash
      if(height>0){
        that.getBlock(height).then(function(block){
          let hashedBlock = JSON.parse(block);
          newBlock.previousBlockHash = hashedBlock.hash;
        });
      }
      ...
      console.log('last part executed!');
